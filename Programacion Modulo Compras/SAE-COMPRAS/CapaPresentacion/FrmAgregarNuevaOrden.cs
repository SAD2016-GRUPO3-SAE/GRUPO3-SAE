...
{
    public partial class FrmAgregarNuevaOrden : Form
    {
        Ordenes _owner;

        public FrmAgregarNuevaOrden()
        {
            InitializeComponent();
        }

        public FrmAgregarNuevaOrden(Ordenes owner)
        {
            _owner = owner;
            InitializeComponent();
        ...
        private void buttonGuardar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    mensaje = NOrdenes.Insertar(this.comboBoxClientes.Text,  this.dateTimePickerFechaOrdenado.Value.ToString("dd/MM/yyyy"),
                    fecha_requerido);
                    ...
                    if (mensaje == "Y")
                    {
                        for (int fila = 0; fila < dataGridViewProductos.Rows.Count; fila++)
                        {
                            mensaje = NOrdenDetalles.Insertar(dataGridViewProductos.Rows[fila].Cells["producto"].Value.ToString(),
