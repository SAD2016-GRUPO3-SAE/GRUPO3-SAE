    ...
    public partial class Proveedores : Form
    {
        ...
        public void Mostrar()
        {
            try
            {
                ...
                this.dataGridViewClientes.DataSource = NProveedores.Mostrar(numeroPagina, registrosPorPagina);
        ...
        public void Buscar()
        {
            try
            {
                this.dataGridViewClientes.DataSource = NProveedores.Buscar(this.textBoxBuscarNombre.Text);
            }
            catch (Exception ex)
            {
                MensajeError(ex.Message);
        ...
        private void pictureBox3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    FrmEditarProveedor editarCliente = new FrmEditarProveedor(this);
        ...
        private void pictureBox2_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.dataGridViewClientes.Rows.Count > 0)
                {
                    DialogResult confirmacion = MessageBox.Show("¿Seguro deseas eliminar este socio?", "Eliminar SOCIO",
                    MessageBoxButtons.OKCancel, MessageBoxIcon.Information);

                    if (confirmacion == DialogResult.OK)
                    {
                        String mensaje = NProveedores.Eliminar(Convert.ToInt32(ObtenerSeleccion().Cells["ID"].Value));
                        if (mensaje == "Y")
                        {
                            Mensaje(String.Format("El Proveedor {0} ha sido ELIMINADO",
                                Convert.ToString(ObtenerSeleccion().Cells["PROVEEDOR"].Value)));
                            Refrescar();
                        }
                        else
                        {
        ...
        {
            FrmAgregarNuevoProveedor nuevoCliente = new FrmAgregarNuevoProveedor(this);
