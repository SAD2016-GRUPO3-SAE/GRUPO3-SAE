...
Public Partial Class FrmEditarOrdenPendiente
	Inherits Form
	...
	Public Sub New(owner As Ordenes)
		...
		Try
			...
			Me.dateTimePickerFechaOrdenado.Value = Convert.ToDateTime(_owner.ObtenerSeleccionPedidos().Cells("ORDENADO").Value)
			...
			If [String].IsNullOrWhiteSpace(_owner.ObtenerSeleccionPedidos().Cells("REQUERIDO").Value.ToString()) Then
				Me.checkBoxSinFecha.Checked = True
			Else

				Me.checkBoxSinFecha.Checked = False
				Me.dateTimePickerFechaRequerido.Value = Convert.ToDateTime(_owner.ObtenerSeleccionPedidos().Cells("REQUERIDO").Value)
			End If
	...
	Private Sub buttonGuardar_Click(sender As Object, e As EventArgs)
		...
		Try
			...
			If Me.dataGridViewProductos.Rows.Count > 0 Then
				mensaje = NOrdenes.Editar(Convert.ToInt32(_owner.ObtenerSeleccionPedidos().Cells(1).Value), Me.comboBoxClientes.Text, Me.comboBoxTransportes.Text, Me.comboBoxEmpleados.Text, Me.dateTimePickerFechaOrdenado.Text, fecha_requerido, _
					Nothing, Convert.ToDecimal(Me.textBoxCostoEnvio.Text))
